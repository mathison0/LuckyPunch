...
// LuckyPunch v5 — 예측 수비 (v4 기반)
//
// v4끼리의 미러전을 프레임 단위로 추적하면 랠리는 거의 항상 3타 교환으로
// 끝난다: 서버 스매시 → 수비자 공중 요격·반격 → 서버 재반격 → 득점.
// 승부는 결국 "재반격이 꽂힐 때 수비자가 어디 서 있었는가"로 갈린다.
// v4의 수비는 코트 중앙 고정 대기라 여기서 진다.
//
// v5의 수비: 공이 상대 쪽에 있을 때 상대의 접촉 시점을 추정하고, 그 접촉
// 상태에서 가능한 수(파워히트 6종 + 몸통 리턴)의 **낙하 지점**을 전부
// 시뮬레이션한다. 그리고 "가장 받기 어려운 수의 낙하 지점까지의 부족
// 거리"가 최소가 되는 위치(maximin)에 미리 가서 선다. 상대가 내 위치에서
// 가장 먼 곳을 노리는 봇(v4류)이라면 이것이 정확한 최적 대응이다.
//
// 만들면서 측정으로 배운 것 (전부 오프라인 하네스 수백 세트 기준):
//  - 위협 커버리지를 "비행 중 요격 가능성"으로 재면 과대평가가 된다.
//    공중 요격은 점프 선행 프레임이 필요해서 실제로는 거의 못 잡는다.
//    믿을 수 있는 수비는 낙하 지점 선점뿐이다 → 위협을 낙하 지점으로 요약.
//  - 접촉 전까지 도달 가능한 위치로 후보를 제한해야 한다. 제한을 풀면
//    이론상 좋아 보여도 접촉 임박 시 목표가 튀어 다니며 손해를 본다.
//  - 상대가 지상일 때도 예측을 전량 반영하는 쪽(TRUST=1)이 실측 지연(1)
//    환경에서 최선이었다: 전 상대 최소 75%, v3·v4 상대 100%. 지연이 2로
//    커지면 약한 상대에게 뒤집히는 트레이드오프가 있지만, 반영 지연 1은
//    브라우저에서 직접 계측한 값이라 lag1을 우선했다.
//  - "공중에서 떠나가는 공을 쫓지 않고 복귀"(DEF_AIR)도 시도했으나
//    예측 수비와 조합하면 네트 근처 커버를 버리는 셈이라 순손해 → OFF.
//    다만 공중 접촉 탐색이 상대 코트의 공을 가짜 접촉으로 잡던 버그
//    수정(내 코트 제한)은 유지했다.
//
// 공격 로직은 v4와 동일하다 (접촉 시점 후보 비교, 세팅 OFF).
