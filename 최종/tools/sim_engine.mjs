...
export class Engine {
  ...
  frozenStep() {
    // Between rallies the tournament engine keeps ticking/calling bots while
    // physics is paused. Inputs are not applied to player/ball state here.
    this.frame++;
    this.lastWorldEvents = [];
    return null;
  }
...
export function playSet({
  ...
  while (engine.scores[0] < winningScore && engine.scores[1] < winningScore && engine.frame < maxFrames) {
    ...
    if (result) {
      ...
      // Tournament rule: only the match-opening server is random.
      // After a point, preserve the original game's rally flow: the side that
      // conceded the point receives the next serve.
      const loser = result.point ^ 1;
      engine.resetRound(loser === 1);
    }
  }
...
// Approximate tournament set runner. It applies the published 10-point,
// 4-minute lead / 5-minute golden-ball rule in engine-frame time. This harness
// models the known round-reset frozen ticks (30 initial / 35 between rounds by
// default). Use it for regression and strategy checks; the released tournament
// repository remains authoritative for exact wall-clock timing.
export function playTournamentSet({
  leftBot,
  rightBot,
  seed = 1,
  latencyLeft = 1,
  latencyRight = 1,
  winningScore = 10,
  regulationFrames = 25 * 60 * 4,
  goldenEndFrames = 25 * 60 * 5,
  initialFreezeFrames = 30,
  betweenRoundFreezeFrames = 35,
  trace = false,
}) {
  const engine = new Engine(seed);
  const left = new BotScheduler(leftBot, 'LEFT', latencyLeft);
  const right = new BotScheduler(rightBot, 'RIGHT', latencyRight);
  engine.resetRound(null); // opening serve only: random from the seeded RNG
  const points = [];
  let golden = false;
  let endReason = null;

  const checkClock = () => {
    if (!golden && engine.frame >= regulationFrames) {
      if (engine.scores[0] !== engine.scores[1]) {
        endReason = 'regulation-lead';
        return true;
      }
      golden = true;
    }
    if (golden && engine.frame >= goldenEndFrames) {
      endReason = 'five-minute-draw';
      return true;
    }
    return false;
  };
  ...
  const runFrozen = (n) => {
    for (let i = 0; i < n; i++) {
      left.beforeFrame(engine);
      right.beforeFrame(engine);
      engine.frozenStep();
      if (checkClock()) return true;
    }
    return false;
  };

  if (runFrozen(initialFreezeFrames)) {
    const winner = engine.scores[0] === engine.scores[1] ? -1 : engine.scores[0] > engine.scores[1] ? 0 : 1;
    return { winner, score:[...engine.scores], frames:engine.frame, endReason, golden, points: trace ? points : undefined };
  }

  while (engine.frame < goldenEndFrames) {
    if (engine.scores[0] >= winningScore || engine.scores[1] >= winningScore) {
      endReason = 'score-10';
      break;
    }

    // At 4:00, a non-tied score ends immediately; a tie enters golden ball.
    if (checkClock()) break;
    ...
    if (result) {
      ...
      const loser = result.point ^ 1;
      engine.resetRound(loser === 1);
      if (runFrozen(betweenRoundFreezeFrames)) break;
    }
  }

  if (!endReason) endReason = 'five-minute-draw';
  const winner = engine.scores[0] === engine.scores[1]
    ? -1
    : engine.scores[0] > engine.scores[1] ? 0 : 1;
  return {
