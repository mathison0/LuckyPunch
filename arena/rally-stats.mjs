...
function runSet(meFile, oppFile, seed, jit, meLeft, agg) {
  ...
  const pending = [null, null]; const ticks = [0, 0];
  function getInput(i) {
    ...
    const snap = { tick: ticks[i], side: isP2 ? 'RIGHT' : 'LEFT',
  ...
  while (scores[0] < 10 && scores[1] < 10) {
    ...
    const touches = [0, 0]; const prevCol = [false, false];
    let prevOnLeft = null, rf = 0, loser = -1;
    while (true) {
      ...
      for (let i = 0; i < 2; i++) {
        ...
        if (col && !prevCol[i]) { touches[i]++; if (touches[i] >= 5) loser = i; }
        prevCol[i] = col;
      }
    ...
    agg.rallies++; if (iWon) agg.wins++;
    if (meServes) { agg.srvR++; if (iWon) agg.srvW++; }
    else { agg.rcvR++; if (iWon) agg.rcvW++; }
    isP2Serve = rng() < 0.5;
  }
...
const OPPS = ['LuckyPunch_v1.js','LuckyPunch_v2.js','LuckyPunch_v3.js','LuckyPunch_v4.js','LuckyPunch_v5.js','LuckyPunch_v6.js'];
const pct = (w, n) => (n ? ((w / n) * 100).toFixed(1).padStart(5) + '%' : '    —');
console.log('v7 상대별 랠리 승률 (lag1, 지터{0,8}, 양 진영, 상대당 48세트)');
console.log('상대 |  랠리 승률 (표본)   | 서브 랠리 | 리시브 랠리 | 세트');
for (const opp of OPPS) {
  const agg = { sets: 0, setWins: 0, rallies: 0, wins: 0, srvR: 0, srvW: 0, rcvR: 0, rcvW: 0 };
  for (let i = 0; i < 12; i++)
    for (const jit of [0, 8])
      for (const L of [true, false])
        runSet('LuckyPunch_v7.js', opp, 61000 + i * 7919, jit, L, agg);
  const name = opp.replace('LuckyPunch_', '').replace('.js', '');
  console.log(`${name.padEnd(4)} | ${pct(agg.wins, agg.rallies)} (${String(agg.rallies).padStart(4)}랠리) |  ${pct(agg.srvW, agg.srvR)}  |   ${pct(agg.rcvW, agg.rcvR)}  | ${pct(agg.setWins, agg.sets)}`);
}
