...
// LuckyPunch v8 — 첫 서브 고속 아치 (v7 기반)
//
...
// 이기는" 결정론 패턴도 깨진다.
//
// v8은 랠리 첫 서브만 고속 아치(|vx|=20, y=-1)로 보낸다. v7의 나머지
// 동작은 그대로 둔다. 긴 수직 궤적이 v3·v4 계열의 가장 이른 접촉·점프
// 타이밍을 무너뜨리면서도 v5 이후의 maximin 수비 상대 성적은 보존했다.
// 느린 아치(|vx|=10)는 기본 AI 상대로 크게 회귀해 기각했다.
...
var OPENING_SERVE = 1; // 첫 서브 한 번만 고속 아치로 바꾼다
var SERVE_X_MAG = 1; // 빠른 20px/frame — 느린 10px는 기본 AI 상대로 회귀
var SERVE_Y_DIR = -1; // 위로 띄워 네트 기둥을 안전하게 넘긴다
...
var rallyCrossings = 0; // 이번 랠리에서 공이 네트를 건넌 횟수
var tickCounter = 0;
    ...
    if (contact >= 0 && contact <= lag + HIT_LEAD_FRAMES) {
      ...
      if (onMySideAtContact) {
        ...
        var best = openingServe
          ? { score: 0, hit: 1, xMag: SERVE_X_MAG, yDir: SERVE_Y_DIR }
          : chooseHit(
              c.x,
              c.y,
              c.vy,
