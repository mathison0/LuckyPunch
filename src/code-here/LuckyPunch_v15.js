...
/*
 * LuckyPunch v15 — receive counterattack state machine
 * ---------------------------------------------------------------------------
 * v14의 공격/수비 적응을 유지하고, 리시브에서 직접 반격이 연속으로 막히면
 * 첫 터치를 자기 코트에 세운 뒤 두 번째 공격으로 전환하는 상태머신을 더한다.
 *
 * Design goals:
...
var DEF_ADAPTIVE = 1;
var CONTACT_TIMING_MODE = -1; // -1=상태머신, 0=복수접촉, 1=가장이른 접촉
var timingTrials = [0, 0]; // 0=MULTI, 1=EARLY
var timingWins = [0, 0];
var activeContactTimingMode = 0;
var timingPrevSelf = -1;
var timingPrevOpp = -1;
var timingPriorWasReceive = false;
...
var ATTACK_TEMPO_ADAPTIVE = 1;
var serveAttackTrials = [0, 0]; // 0=즉시 점프, 1=지상 셀프세팅
var serveAttackWins = [0, 0];
var activeServeAttackMode = -1;
var attackPrevSelf = -1;
var attackPrevOpp = -1;
var COUNTER_RECEIVE = 0;
var COUNTER_SET_PLANNED = 1;
var COUNTER_SECOND_ATTACK = 2;
var COUNTER_RECOVER = 3;
var counterPhase = COUNTER_RECEIVE;
var receiveTempoTrials = [0, 0]; // 0=즉시 반격, 1=통제 셀프세팅
var receiveTempoWins = [0, 0];
var requestedReceiveTempo = 0;
var receiveSetCommitted = false;
var counterPriorWasReceive = false;
var counterInitialized = false;
var counterPrevSelf = -1;
var counterPrevOpp = -1;
// Deep corner receives already produce a safe direct clear.  Reserve the
// controlled set for contacts closer to the attack zone.
var RECEIVE_SET_MIN_CONTACT_X = 70;
...
function selectServeAttackMode() {
  // 공격 우선: 즉시 점프가 이기는 동안에는 세팅을 탐색하지 않는다. 실제
  // 득점률이 50% 아래로 내려간 상대에게만 셀프세팅을 시험한다.
  if (serveAttackTrials[0] === 0) return 0;
  const immediateRate = serveAttackWins[0] / serveAttackTrials[0];
  if (serveAttackTrials[1] === 0) return immediateRate < 0.5 ? 1 : 0;
  const setRate = serveAttackWins[1] / serveAttackTrials[1];
  return setRate > immediateRate ? 1 : 0;
}

function selectContactTimingMode() {
  // 리시브 랠리에서 EARLY를 먼저 시험하고, 다음 리시브에서 MULTI를 시험한다.
  // 같은 승률이면 최신·가변 공격에 강한 MULTI를 우선한다.
  if (timingTrials[1] === 0) return 1;
  if (timingTrials[0] === 0) return 0;
  const multiRate = timingWins[0] / timingTrials[0];
  const earlyRate = timingWins[1] / timingTrials[1];
  return earlyRate > multiRate ? 1 : 0;
}

function effectiveContactTimingMode() {
  return CONTACT_TIMING_MODE >= 0
    ? CONTACT_TIMING_MODE
    : activeContactTimingMode;
}

function selectReceiveTempo() {
  // 직접 반격 두 번으로 상대가 이 템포를 확실히 막는지 먼저 확인한다.
  // 0승 2패 수준일 때만 통제 세팅을 시험해 강한 직접 반격 상대에게
  // 불필요한 탐색 비용을 지불하지 않는다.
  if (receiveTempoTrials[0] < 2) return 0;
  const directRate = receiveTempoWins[0] / receiveTempoTrials[0];
  if (receiveTempoTrials[1] === 0) return directRate < 0.45 ? 1 : 0;
  const setRate = receiveTempoWins[1] / receiveTempoTrials[1];
  return setRate > directRate ? 1 : 0;
}

function beginCounterRally(ownServe) {
  counterPriorWasReceive = !ownServe;
  requestedReceiveTempo = ownServe ? 0 : selectReceiveTempo();
  receiveSetCommitted = false;
  counterPhase = COUNTER_RECEIVE;
}
...
function chooseBodyContactGeometry(c, bf) {
  const receiveSetWindow =
    counterPriorWasReceive &&
    requestedReceiveTempo === 1 &&
    MEM.ownTouchEstimate === 0 &&
    (counterPhase === COUNTER_RECEIVE ||
      counterPhase === COUNTER_SET_PLANNED);
  // A normal body receive stands well behind the ball and therefore clears it
  // immediately.  Controlled receive uses a near-centre body angle so the
  // first touch rises almost vertically and remains on our side.
  const offsets = receiveSetWindow
    ? [3, 4, 6, 9, 12, 18, 25, 30]
    : [9, 12, 18, 25, 30];
  const selfSetAllowed =
    !MEM.selfSetUsed &&
    MEM.ownTouchEstimate <= 1 &&
    (bf.x < 132 || (receiveSetWindow && bf.x < C.NET_X - 26)) &&
    (!receiveSetWindow || bf.x >= RECEIVE_SET_MIN_CONTACT_X);
  let best = null;
...
function defensiveStandbyX(c) {
  ...
  const timingMode = effectiveContactTimingMode();
  if (timingMode === 1 && contacts.length > 1) contacts.length = 1;
  ...
  if (contacts.length) {
    ...
    if (threats.length) {
      ...
      const trust = timingMode === 1
        ? EARLY_DEF_TRUST
        : DEF_ADAPTIVE === 1 && activeDefenseMode === 1
        ? 1
        : EARLY_DEF_TRUST;
...
function planCore(c, estimatedSelfVy) {
  ...
  if (landingOwn) {
    ...
    const ownServe = c.flip ? c.isPlayer2Serve : !c.isPlayer2Serve;
    const likelyOwnServe =
      ownServe && c.ball.x < 105 && abs(c.ball.vx) <= 1 && c.ball.y < 165;
    if (likelyOwnServe && activeServeAttackMode < 0) {
      activeServeAttackMode = ATTACK_TEMPO_ADAPTIVE === 1
        ? selectServeAttackMode()
        : 0;
    }
    const controlledServeSet =
      likelyOwnServe &&
      activeServeAttackMode === 1 &&
      groundContact &&
      groundContact.geometry &&
      groundContact.geometry.type === 'SELF_SET';
    const controlledReceiveSet =
      counterPriorWasReceive &&
      requestedReceiveTempo === 1 &&
      (counterPhase === COUNTER_RECEIVE ||
        counterPhase === COUNTER_SET_PLANNED) &&
      ...
      groundContact.geometry &&
      groundContact.geometry.type === 'SELF_SET';
    if (controlledReceiveSet) {
      if (!receiveSetCommitted) {
        receiveSetCommitted = true;
        counterPhase = COUNTER_SET_PLANNED;
      }
    }
...
function decide(snapshot) {
  ...
  MEM.decisions++;
  const ownServeNow = c.flip ? c.isPlayer2Serve : !c.isPlayer2Serve;
  const counterScoreChanged = counterPrevSelf >= 0 &&
    (c.scoreSelf !== counterPrevSelf || c.scoreOpp !== counterPrevOpp);
  if (counterScoreChanged) {
    if (counterPriorWasReceive) {
      const used = receiveSetCommitted ? 1 : 0;
      receiveTempoTrials[used]++;
      if (c.scoreSelf > counterPrevSelf) receiveTempoWins[used]++;
    }
    beginCounterRally(ownServeNow);
    counterInitialized = true;
  } else if (!counterInitialized) {
    beginCounterRally(ownServeNow);
    counterInitialized = true;
  }
  counterPrevSelf = c.scoreSelf;
  counterPrevOpp = c.scoreOpp;
  const timingScoreChanged = timingPrevSelf >= 0 &&
    (c.scoreSelf !== timingPrevSelf || c.scoreOpp !== timingPrevOpp);
  if (timingScoreChanged) {
    if (timingPriorWasReceive) {
      timingTrials[activeContactTimingMode]++;
      if (c.scoreSelf > timingPrevSelf) timingWins[activeContactTimingMode]++;
    }
    activeContactTimingMode = ownServeNow
      ? 0
      : selectContactTimingMode();
    timingPriorWasReceive = !ownServeNow;
    timingInitialized = true;
  } else if (!timingInitialized) {
    activeContactTimingMode = ownServeNow ? 0 : selectContactTimingMode();
    timingPriorWasReceive = !ownServeNow;
    timingInitialized = true;
  }
  timingPrevSelf = c.scoreSelf;
  timingPrevOpp = c.scoreOpp;
  if (attackPrevSelf >= 0 &&
      (c.scoreSelf !== attackPrevSelf || c.scoreOpp !== attackPrevOpp)) {
    if (activeServeAttackMode >= 0) {
      serveAttackTrials[activeServeAttackMode]++;
      if (c.scoreSelf > attackPrevSelf) serveAttackWins[activeServeAttackMode]++;
    }
    activeServeAttackMode = -1;
  }
  attackPrevSelf = c.scoreSelf;
  attackPrevOpp = c.scoreOpp;
  ...
  if (counterPhase === COUNTER_SET_PLANNED &&
      MEM.selfSetUsed && MEM.ownTouchEstimate > 0) {
    counterPhase = COUNTER_SECOND_ATTACK;
  }
  if (counterPhase === COUNTER_SECOND_ATTACK && c.ball.x > C.NET_X) {
    counterPhase = COUNTER_RECOVER;
  }
