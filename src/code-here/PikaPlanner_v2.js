...
function evaluateBodyContact(c, bf, playerX, contactFrame, oppVy) {
  ...
  const allowSelfSet =
    !MEM.selfSetUsed &&
    MEM.ownTouchEstimate < CFG.SELF_SET_MAX_TOUCH_EST &&
...
function findBestGroundReceive(c, freeTr, oppVy) {
  ...
  for (let i = 0; i < limit; i++) {
    ...
    for (let oi = 0; oi < CFG.RECEIVE_OFFSETS.length; oi++) {
      const off = CFG.RECEIVE_OFFSETS[oi];
      const px = clamp(bf.x - off, C.LEFT_MIN, C.LEFT_MAX);
      if (abs(bf.x - px) > C.PLAYER_HALF) continue;
      if (!selfCanReachX(c, px, t)) continue;
      const ev = evaluateBodyContact(c, bf, px, t, oppVy);
      if (!ev) continue;
      // Earlier equally-good receive is safer, but outgoing trajectory value dominates.
      ev.score -= t * 0.20 + abs(px - c.self.x) * 0.01;
      ev.contactFrame = t;
      ev.ballFrame = bf;
...
function planCore(c, selfVy, oppVy) {
  ...
  if (land.x < C.NET_X) {
    ...
    if (ground) {
      return {
        type: ground.type,
        x: moveToward(c.self.x, ground.playerX),
        y: 0,
        hit: 0,
        targetX: ground.playerX,
