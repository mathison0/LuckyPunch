...
// LuckyPunch v11 — 서브 첫 접촉 공중 세팅 (v10 기반, lag1 공격형 변형)
//
// v6에서 확인한 것: 상대의 위치·속도를 외삽해 "있을 곳의 반대편"을 노리는
// 공격이 가능하다. 그렇다면 우리도 당할 수 있다 — 우리 수비 대기는
// maximin 위치에 서서 정지하는데, 정지는 속도 0으로 완벽하게 예측된다.
//
// v7의 대응: 수비 대기 중 "와리가리" 진동.
//   - maximin 위치까지는 직행한다 (커버리지가 우선).
//   - 도착하면 그 주변 ±OSC_AMP 안에서 계속 움직인다. 방향 전환 시점은
//     Math.random으로 정하므로, 상대가 우리 코드를 그대로 시뮬레이션해도
//     못 맞춘다.
//   - 항상 |속도|=6으로 움직이므로 속도 외삽 공격의 예측 오차가
//     최대 6*(외삽구간)px 만큼 생긴다. 진폭이 작아 maximin 커버리지
//     손실은 OSC_AMP 이하로 제한된다.
//
// 적용 범위는 수비 대기(공이 상대 쪽으로 갈 때)뿐이다. 공격·리시브·공중
// 동작은 v6과 동일하다. 무작위성 덕분에 집안 맞대결의 "서브 넣은 쪽이
// 이기는" 결정론 패턴도 깨진다.
...
var OSC_AMP = 20; // 수비 대기 진동 폭 (px)
var JINK_SAFE_FRAMES = 22; // 상대 접촉이 이보다 멀 때만 진동. 가까우면 정확 수렴
var OSC_MIN_TICKS = 2; // 방향 유지 최소 tick 수
var OSC_RAND_TICKS = 4; // 추가로 0~이만큼 무작위 유지
...
var DEF_AIR = 0; // 전역 공중 복귀는 재측정에서도 P2+P3보다 약해 OFF
var DEF_PREDICT = 1; // 1이면 maximin 예측 수비, 0이면 코트 중앙 대기
var CONTACT_GAIN = 60; // 늦은 접촉이 이만큼 더 좋아야 늦춘다
var SERVE_2PLY = 1; // 첫 서브만 상대 요격·반격까지 보고 슛을 고른다
var COUNTER_WEIGHT = 1; // 0.3/0.6/1.0 스윕에서 1.0이 최선
var DEF_AIR_SERVE = 1; // 첫 서브 직후에만 공중에서 수비 위치로 복귀
var GROUND_SERVE_SET = 0; // 첫 서브를 점프 없이 몸으로 받아 자기 코트에 띄운다
var AIR_SERVE_SET = 1; // 첫 서브는 점프하되 파워히트 없이 자기 코트에 띄운다
...
var oscDir = 1; // 수비 진동의 현재 방향
var oscFlipAt = 0; // 이 tick이 되면 방향을 다시 뽑는다
...
var rallyCrossings = 0; // 현재 랠리에서 공이 네트를 건넌 횟수
var tickCounter = 0;
 ...
 * 첫 서브 전용 2-ply 채점.
 *
 * 내 6가지 파워히트 각각에 대해 상대의 가장 이른 점프 요격을 찾고, 그
 * 접촉에서 상대가 chooseHit()로 고를 최선의 반격까지 시뮬레이션한다.
 * 기존 1수 점수에 "그 반격이 나에게 얼마나 쉬운가"를 더한다.
 *
 * DEF_AIR_SERVE가 켜졌다면 상대 요격 전까지 planDefense의 목표 쪽으로
 * 실제로 복귀할 수 있는 거리도 내 예상 위치에 반영한다.
 */
function chooseHitServe(
  ballX,
  ballY,
  ballVy,
  playerX,
  ...
  for (var xMag = 1; xMag >= 0; xMag--) {
    for (var yDir = 1; yDir >= -1; yDir--) {
      var v = powerHitVelocity(ballX, ballVy, xMag, yDir);
      var shot = evaluateShot(ballX, ballY, v.vx, v.vy, oppOnRight, oppAtHit);
      var onePly = scoreShot(shot, oppOnRight, oppAtHit);
      var shotTraj = simulateFrom(ballX, ballY, v.vx, v.vy);
      var intercept = findJumpContact(
        shotTraj,
        1,
        oppAtHit,
        !isLeft,
      ...
      // 요격 자체가 없으면 직접 득점 후보다. 1수 점수가 이미 큰 보상을
      // 주므로 2수 항은 중립으로 둔다.
      var counterTerm = 0;
      if (intercept !== null) {
        var myAtCounter = playerX;
        if (DEF_AIR_SERVE === 1) {
          var recovery = planDefense(
            shotTraj,
            0,
            playerX,
            isLeft,
          ...
          if (recovery !== null) {
            var maxMove = PLAYER_SPEED * intercept.contact;
            myAtCounter = clamp(
              recovery.x,
              playerX - maxMove,
              playerX + maxMove
            );
            myAtCounter = clamp(myAtCounter, reachMin, reachMax);
          }
        }

        var c = shotTraj[intercept.contact];
        var oppHitX = clamp(c.x, oppReachMin, oppReachMax);
        var counter = chooseHit(
          c.x,
          c.y,
          c.vy,
          oppHitX,
        ...
        counterTerm = -counter.score;
      }

      var total = onePly + COUNTER_WEIGHT * counterTerm;
      if (best === null || total > best.score) {
        best = { score: total, hit: 1, xMag: xMag, yDir: yDir };
      }
    }
  }
  return best;
...
/**
 * 수비 대기용 이동: targetX까지는 직행, 근처에 오면 ±OSC_AMP 안에서
 * 무작위 왕복한다. 정지(속도 0)는 위치·속도 외삽 공격에 완벽히 읽히므로
 * 대기 중에도 항상 움직여서 예측 오차를 강제한다.
 */
function jinkAround(targetX, myX, lo, hi, amp) {
  var dx = targetX - myX;
  if (amp < PLAYER_SPEED) {
    return walkTo(targetX, myX); // 여유 없음 — 정지 정밀 수비
  }
  if (Math.abs(dx) > amp) {
    return dx > 0 ? 1 : -1; // 아직 멀다 — 직행 (커버리지 우선)
  }
  // 진동 모드: 무작위 시점마다 방향을 다시 뽑는다
  if (tickCounter >= oscFlipAt) {
    oscDir = Math.random() < 0.5 ? -1 : 1;
    oscFlipAt =
      tickCounter + OSC_MIN_TICKS + Math.floor(Math.random() * (OSC_RAND_TICKS + 1));
  }
  // 진폭 경계·코트 경계에서 반사
  if (myX + oscDir * PLAYER_SPEED > Math.min(targetX + amp, hi)) {
    oscDir = -1;
  } else if (myX + oscDir * PLAYER_SPEED < Math.max(targetX - amp, lo)) {
    oscDir = 1;
  }
  return oscDir;
}
...
function think(s) {
  ...
  var myServe = isLeft ? !s.meta.isPlayer2Serve : s.meta.isPlayer2Serve;
  var lag = LAG_FRAMES;
  ...
  if (myState === 1 || myState === 2) {
    ...
    // 공중인데 이 공을 칠 수 없다면(contact < 0), 공을 쫓는 건 무의미하다.
    //
    // 특히 내 코트로 오는 공: xToBall은 공의 "2프레임 뒤 위치"를 쫓는데,
    // 빠른 반격은 나를 지나쳐 등 뒤에 떨어진다. 공을 향해 걷다가 낙하점과
    // 정반대로 가버리는 버그가 v5·v6에 있었고 (프레임 트레이스로 확인 —
    // 반격 낙하점이 x=36인데 x=184까지 걸어감), 여기서 고친다:
    // 착지 후 받을 위치(낙하점 옆)로 즉시 걷는다.
    if (contact < 0 && landsOnMySide) {
      var towardNetAir = isLeft ? 1 : -1;
      var recvAir = clamp(
        landX - towardNetAir * RECEIVE_OFFSET,
        reachMin,
        reachMax
      );
      return { x: walkTo(recvAir, myX), y: 0, hit: 0 };
    }
    // 상대 코트로 떠나가는 공을 쫓지 않는 처리(DEF_AIR)는 측정 결과
    // 예측 수비와 조합 시 손해라 기본 OFF다.
    var serveRecovery =
      DEF_AIR_SERVE === 1 && myServe && rallyCrossings <= 1;
    if (
      (DEF_AIR === 1 || serveRecovery) &&
      contact < 0 &&
      !landsOnMySide
    ) {
    ...
    if (contact >= 0 && contact <= lag + HIT_LEAD_FRAMES) {
      ...
      if (onMySideAtContact) {
        var oppAtHit = predictOppX(oppX, contact, isLeft);
        var openingServe =
          myServe && rallyCrossings === 0 && possessionTouches === 0;
        if (AIR_SERVE_SET === 1 && openingServe) {
          // 첫 접촉에서 파워히트를 참아 몸통 리턴으로 자기 코트에 띄운다.
          // 다음 접촉부터는 v10의 2-ply 공격으로 복귀한다.
          return { x: xToBall, y: 0, hit: 0 };
        }
        var best =
          SERVE_2PLY === 1 && openingServe
            ? chooseHitServe(
                c.x,
                c.y,
                c.vy,
            ...
            : chooseHit(c.x, c.y, c.vy, myX, isLeft, oppAtHit);
  ...
  var openingServeGround =
    myServe && rallyCrossings === 0 && possessionTouches === 0;
  if (GROUND_SERVE_SET === 1 && openingServeGround) {
    // 서브 공 바로 아래로 이동해 vx≈0인 몸통 리턴을 만든다. 점프하지 않으므로
    // 첫 접촉 뒤에도 지상 상태를 유지하고, 두 번째 접촉에서 정상 공격한다.
    return {
      x: trackX(traj[Math.min(lag, landIdx)].x, myX),
      y: 0,
      hit: 0,
    };
  }
