...
/*
 * LuckyPunch v16 — score-aware receive recovery state machine
 * ---------------------------------------------------------------------------
 * v15의 공격/수비 적응을 유지한다. 동점이나 리드에서는 직접 반격하고,
 * 한 점이라도 뒤지면 첫 터치를 세운 뒤 두 번째 공격으로 복구를 시도한다.
 *
 * Design goals:
...
var RECEIVE_RECOVERY_DEFICIT = 1;
...
function selectReceiveTempo(scoreDiff) {
  return scoreDiff <= -RECEIVE_RECOVERY_DEFICIT ? 1 : 0;
}

function beginCounterRally(ownServe, scoreDiff) {
  counterPriorWasReceive = !ownServe;
  requestedReceiveTempo = ownServe ? 0 : selectReceiveTempo(scoreDiff);
...
function decide(snapshot) {
  ...
  if (counterScoreChanged) {
    beginCounterRally(ownServeNow, c.scoreSelf - c.scoreOpp);
